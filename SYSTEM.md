...
graph TB
  ...
  bsr -->|"documents.json (shared spec text)"| reader["site/spec-reader/"]
  bsd -->|"behaviours-v5-reader.json (band-boundary build)"| reader
  bsd -->|"timestamped payloads + manifest"| panelview["site/llm-panel-review/ (unlinked)"]
  reader -->|"documents.json"| panelview
  ...
  reader & panelview & proto & meth ==>|"deploy.yml on site/** changes"| cf["Cloudflare Pages"]
...
4. **Site payloads** — `spec-reader/data/documents.json` is shared by the reader and the panel; panel citations carry `exampleBlock` flags anchoring example blocks. The reader's behaviour payload is pinned by literal filename (`site/llm-panel-review/data/behaviours-v5-reader.json`, the band-filtered build): the reader deliberately does not resolve through the panel's manifest, because the panel payloads are unfiltered and the manifest is gitignored. Panel runs emit timestamped payloads + `manifest.json` (latest-by-default, gitignored); the committed `behaviours.json` is the fresh-clone fallback. The compare view generalizes to N documents — every document renders as a pane with a boundary resizer between adjacent panes (a user-registered spec is a first-class pane).
...
5. **Hand-maintained surfaces**: `index.html` inline data (diverged from its prototype source). The reader's behaviour payload is a derived build (`build_site_data.py --threshold=4 --solid-threshold=6` on the committed v5 run), committed at `site/llm-panel-review/data/behaviours-v5-reader.json`; its cell curation lives in `data/panel-cell-curation.json`.
