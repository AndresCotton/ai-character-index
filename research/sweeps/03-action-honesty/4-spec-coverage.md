...
Judged against the behaviour-1 anchors (constitution 3/4 "named explicitly but no dedicated section, no operational test"; model spec 4/4 "dedicated section with an operational invariance rule and worked examples"); re-confirmed unchanged at Gate 4 against the anchored rubric in `research/spec-coverage-depth-rubric.md` (current location: `methodology/spec-coverage-depth-rubric.md`; both specs: prescribed -- quotable conduct rules; the reporting facets are unexampled in both specs, and one facet-edge example does not make the behaviour demonstrated).
