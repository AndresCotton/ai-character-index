...
One file here is **not** index data: `panel-cell-curation.json` — the per-lab cell summaries (verdict/depth/verifiedDate) `engine/panel/build_site_data.py` ships beside the panel payload's passages. The reader (`site/spec-reader/`) renders a derived payload from the same builder (`--threshold=4 --solid-threshold=6` against the committed v5 run), committed at `site/llm-panel-review/data/behaviours-v5-reader.json` and pinned there by literal filename. Nothing here is an index verdict, and nothing reaches `coverage.json`.
