...
## Relationships
`coverage.json` is a frozen ledger — nothing writes it; CI re-verifies every quote in it (`tests/test_coverage_json.py` re-resolves through `engine/spec-cite/cite.py`). The reader's behaviour payload is not a data file here: it is a derived build of `engine/panel/build_site_data.py` (`--threshold=4 --solid-threshold=6` against the committed v5 run), committed at `site/llm-panel-review/data/behaviours-v5-reader.json` and pinned there by literal filename; `panel-cell-curation.json` supplies the builder's per-lab cell rows. `behaviours.json` and `labs.json` are hand-maintained (`engine/notion-sync/` is empty, Phase 3 per PLAN.md); `data/evals.json` was retired by the scope ruling (deleted 2026-08-19). Consumers: `engine/build-spec-reader-data.py` reads `coverage.json` and inlines both spec texts into `site/spec-reader/data/documents.json` (fetched by `site/spec-reader/app.js` and by `site/llm-panel-review/app.js`); with `--user-manifest=` it also folds user-registered specs in. `engine/panel/build_site_data.py` reads `behaviours.json` for behaviour metadata and `panel-cell-curation.json` for the cell rows, and writes timestamped payloads + `manifest.json` into `site/llm-panel-review/data/`. `labs.json` is read by `engine/validate_data.py` (cross-file `lab_id` rule), which gates every file here against `schema/` (+ `engine/test_validate_data.py` pins the gate). `engine/verify-reader-test.mjs` checks the built site payloads, not `data/` directly. `engine/validate_data.py` runs on every PR via `.github/workflows/ci.yml`.
...
graph LR
  ...
  panel -->|"behaviours-v5-reader.json (pinned by filename)"| reader["site/spec-reader/"]
