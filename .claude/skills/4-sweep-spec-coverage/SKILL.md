...
This replaced the manual term sweep: the run is a single logged command with
every verdict banked per judge (a re-run against the same spec version is
comparable cell by cell, where the old sweep was a fresh manual pass), it has no
term-list recall ceiling, and every kept passage carries each judge's named
verdict. Judges are not bit-deterministic; treat rerun stability as an empirical
...
against `methodology/spec-coverage-depth-rubric.md` (0 absent / 1 named / 2 discussed /
