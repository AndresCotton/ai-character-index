---
name: sweep-verify
description: The verify stage of a coverage sweep -- fresh-context audit of published coverage: locator re-resolution, payload identity, local render, gate-log completeness. Produces the final sign-off. Run in a new session or subagent that did not execute the sweep.
---

# The verify stage of a sweep (coverage only)

Input: a behaviour whose coverage gate and publish gate are signed.
...
1. **Quotes.** Every locator in `data/coverage.json` (and the coverage artifact)
...
## The verify gate -- the sweep is done
...
## After the verify gate

Commit the audit record -- `verify.md` plus the signed verify-gate entry in
