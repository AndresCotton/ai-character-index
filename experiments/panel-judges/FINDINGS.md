...
*Status now*: v1 (binary, definition-only) collection COMPLETE for all 9 publishable behaviours and live in the reader-test site overlay (behaviour-hue outlines + vote chips + tooltips + agreement slider + rail marks, branch-only). v2 (ternary + scope clauses) collection 9/11 complete, remainder finishing overnight; the demo flips to v2 data when it lands. Audit sheets distributed, labels expected tomorrow morning. Total spend to date ~$8 across all experiments.
