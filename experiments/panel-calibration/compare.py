...
def main():
    ...
    for a in sys.argv[1:]:
        ...
        variants.append((label, load(runlog, rubric)))
    all_cells = sorted({c for _, v in variants for c in v})
    out = {"variants": [l for l, _ in variants], "cells": {}}
    for cell in all_cells:
        ...
        for label, data in variants:
            ...
            for rank, (s, loc, mv) in enumerate(scored[:15], 1):
                tags = "".join(f" {m}:{mv[m]}" for m in judges)
                snip = TEXT.get(loc, "")[:100].replace("\n", " ")
                mark = ""
                for tloc, tlabel in TARGETS.get(cell, []):
                    if loc == tloc:
                        mark = f"   <<< {tlabel}"
                print(f"  #{rank:2} score {s}/{2*len(judges)} {tags}  {loc.split(' > ', 1)[1]}{mark}\n"
                      f"      {snip}")
            ...
            cellout[label] = {"positive": len(scored), "dist": dict(dist), "top": top,
                              "targets": tinfo, "judges": judges,
                              "scores": {loc: sum(mv.values()) for loc, mv in votes.items()}}
        out["cells"][f"{beh}|{spec}"] = cellout
    (HERE / "compare-latest.json").write_text(json.dumps(out, ensure_ascii=False))
    print(f"\nwrote {HERE / 'compare-latest.json'}")
