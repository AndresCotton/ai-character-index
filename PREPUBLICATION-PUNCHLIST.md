...
- [ ] **Public intake forms.** `submit-eval.yml` is eval-submission intake → out of scope; delete candidate (deleting it also removes one of the hand-synced behaviour-list copies — the form's dropdown). `appeal.yml` is keepable but its "evidence score" wording belongs to the eval workflow → reword to coverage-verdict-only (its structure is already coverage-friendly). `config.yml` (contact link) stays. Downstream: the README "How to contribute" section names the forms and needs updating after any deletion.
