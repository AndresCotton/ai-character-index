...
- [ ] **Scope deletions** *(ruled 2026-08-18)*: out-of-scope material (`data/evals.json`, `research/sources/`, sweep stages 1–3 artifacts) is being deleted via a fully reviewed PR (in flight); the `site/index.html` evidence lens awaits the deferred site-surfaces ruling. `.claude/skills/` scope pass approved (delete `1-sweep-discover/`, `2-sweep-curate/`, `3-sweep-score/` + `references/exclusion-criteria.md`; rescope `5-sweep-publish/` and `6-sweep-verify/` to coverage-only — drafts in `docs/proposals/`; retire the `behaviour-sweep/` orchestrator; strip Notion IDs from `references/locations.md`) — PR in flight. Site-surfaces ruling (keep `spec-reader/` + `llm-panel-review/` + test bench? retire the `index.html` prototype?) deferred until the other changes land. `behaviours-for-adria/` stays in but gets renamed (new name Andrés's call; recommendation `external-review-batch/`). `labs.json` deferred — fold into the spec-identity consolidation later.
...
- [ ] **Panel provenance on main:** commit the canonical panel runlog (or a hash-pinned manifest); derive the substitution provenance text from the runlog instead of a hand-edited string. Urgency: the runlog currently exists only as an UNTRACKED FILE in a local working copy of `experiment/panel-judges` (committed to no branch) — losing that working copy loses the shipped panel data's only provenance
