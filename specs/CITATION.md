...
- The "Behaviours to track" page (`research/core-behaviour-list.md`).
- `data/coverage.json`: each coverage entry stores `locator` + `quote`.
  Locators are re-resolved against `specs/` in CI on every PR
  (`tests/test_coverage_json.py` byte-compares every quote), so a spec update
  that moves text fails loudly instead of silently.
