...
- `pull-latest.sh` still fetches the upstream dated release archives although they exceed the GitHub contents API's 1 MB inline limit and arrive as 0-byte files; the empty artifacts have been removed from the repo and the fetch needs fixing (punch list). Version detection (knowing when upstream has moved past the pinned registry) is an open punch-list item too, and will need its own signal (e.g. a `docs/` listing or CHANGELOG diff).
