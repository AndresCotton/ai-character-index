...
## Purpose
Holds (spec-)coverage sweep artifacts for ten behaviours supplied by an external reviewer ("Adria"). It is a deliberate side channel so that set can be published, revised, and withdrawn without touching the index's own behaviour list, data, or published reader.
...
| `01-helpfulness/` … `08-user-autonomy/` | Eight numbered folders, each with a single `4-spec-coverage.md`: term sweep, resolver-verified excerpt set, per-spec verdict + depth, "considered and not kept" log, mechanical re-check output, coverage-gate checklist (Gate 4 in the batch records) |
...
- Every file is stopped at the coverage gate (Gate 4 in the batch records) with the human spot-read/sign-off unchecked and awaiting Andrés (per `README.md` "Gate status"); publishing to the bench does not close that item.
