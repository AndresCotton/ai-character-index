...
class TestEstimate(unittest.TestCase):
    """Guards the integration-run crash: unpriced judge tag KeyError'd the dry run."""

    def test_unpriced_tag_does_not_crash(self):
        plan = [("b", "constitution", "some-new-model")]
        self.assertGreater(rr.estimate(plan, 2), 0)

    def test_full_behaviour_sums_to_per_behaviour_constant(self):
        plan = [("b", s, "sol") for s in ("constitution", "model-spec")]
        self.assertAlmostEqual(rr.estimate(plan, 2), rr.EST["sol"])
